...
Given(/^I have purchase two products with a total cost of (\w+) (\d+)\.(\d+)$/) do |_currency, amount, cents|
  amount = amount + '.' + cents
...
Given(/^I will pay using (\w+) with (\d+) days to expire$/) do |type, daysToExpire|
  if type.downcase == 'boleto'
    ...
    @boleto.bankNumber = 1
    @boleto.nossoNumero = 989_790
    @boleto.transactionReference = rand(389_546_232)
